...
#line 12 "main.c" /0
 ...
 {
 int i,j,k;	
 printf("MECON=0X%BX, PADR=0X%BX \r\n", (*(unsigned char volatile xdata *)0x8300), (*(unsigned char volatile xdata *)0x8305));
 for(k=0x30;k<0x40;k++)
 {
  (*(unsigned char volatile xdata *)0x8305) = k;
 
 printf("MECON=0X%BX, PADR=0X%BX \r\n", (*(unsigned char volatile xdata *)0x8300), (*(unsigned char volatile xdata *)0x8305));
 printf("code:\r\n");
 
 for(j=0x30;j<(*(unsigned char volatile xdata *)0x8305);j++)
 {
 FlashCodeErase(j,1);		 
 writeHex(j);
 printf("\r\n");
 for(i=0;i<128;i++)buf[i]=j;
 FLASHCodeBufferWrite(buf,j*128,128);
 for(i=0;i<128;++i)rbuf[i]=0;
 FLASHCodeBufferRead(rbuf,j*128,128);
 for(i=0;i<128;++i)
 printf("%BX ",rbuf[i]);
 printf("\r\n");
 }
 ...
 for(j=0;j<(0x40-(*(unsigned char volatile xdata *)0x8305));j++)	 
 {
 FlashDataErase(j,1);
 writeHex(j);
 printf("\r\n");
 for(i=0;i<128;i++)buf[i]=j;
 FLASHDataBufferWrite(buf,128*j,128);
 for(i=0;i<128;i++)rbuf[i]=0;
 FLASHDataBufferRead(rbuf,128*j,128);
 for(i=0;i<128;++i)
 printf("%BX ",rbuf[i]);
 printf("\r\n");
 }
 ...
 unsigned int i,k;
 unsigned char j;	
  (*(unsigned char volatile xdata *)0x8305)=0x80;
 printf("MECON=0X%BX, PADR=0X%BX \r\n", (*(unsigned char volatile xdata *)0x8300), (*(unsigned char volatile xdata *)0x8305));
 printf("code 校验和\r\n");	
 for(i=0;i<(*(unsigned char volatile xdata *)0x8305);++i)
 {
  (*(unsigned char volatile xdata *)0x8306)=((i*128)&0xff);
  (*(unsigned char volatile xdata *)0x8307)=(((i*128)>>8)&0xff);
  (*(unsigned char volatile xdata *)0x8301)=0x0c;
 printf("i=%x, %bx%bx", i,(*(unsigned char volatile xdata *)0x8309),(*(unsigned char volatile xdata *)0x8308));
 ...
 FLASHCodeBufferRead(rbuf,(i*128),128);
 k=0;
 for(j=0;j<128;++j)
 {
 k += rbuf[j];
 ...
 unsigned char j;	
 printf("MECON=0X%BX, PADR=0X%BX \r\n", (*(unsigned char volatile xdata *)0x8300), (*(unsigned char volatile xdata *)0x8305));
  (*(unsigned char volatile xdata *)0x8305)=0x30;
 printf("MECON=0X%BX, PADR=0X%BX \r\n", (*(unsigned char volatile xdata *)0x8300), (*(unsigned char volatile xdata *)0x8305));
 printf("data 校验和");		
 
 for(i=0;i<((0x80)-(*(unsigned char volatile xdata *)0x8305));++i)
 {	
  (*(unsigned char volatile xdata *)0x8306)=(((i)*128)&0xff); 
  (*(unsigned char volatile xdata *)0x8307)=((((i)*128)>>8)&0xff); 
  (*(unsigned char volatile xdata *)0x8301)=0x08;
 printf("i=%x, %bx%bx", i, (*(unsigned char volatile xdata *)0x8309),(*(unsigned char volatile xdata *)0x8308));
 
 
 
 for(k=0;k<128;++k)rbuf[k]=0;
 FLASHDataBufferRead(rbuf,(i*128),128);
 k=0;
 for(j=0;j<128;++j)
 {
 k += rbuf[j];
