...
#line 12 "main.c" /0
 ...
 for(i=0;i<128;++i)buf[i]=i+0X80;
 writeData2EEPROM(buf,0,128);
 for(i=0;i<128;++i)rbuf[i]=0;
 readDatafromEEPROM(rbuf,0,128);
 for(i=0;i<128;i++)
 {
 printf("%bx ", rbuf[i]);
 }
 ...
 for(i=0;i<128/128;++i)
 {
  ...
  (*(unsigned char volatile xdata *)0x8301)=0x4c;
 printf("\r\n%bx%bx", (*(unsigned char volatile xdata *)0x8309),(*(unsigned char volatile xdata *)0x8308));
 
 
 for(k=0;k<128;++k)rbuf[k]=0;
 readDatafromEEPROM(rbuf,(i*128),128);
 k=0;
 for(j=0;j<128;++j)
 {
 k += rbuf[j];
